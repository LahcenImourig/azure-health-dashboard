...
# If fetching fails, the app still starts (secrets will be missing but health check passes).
...
if [ -n "$SECRETS_ARN" ]; then
  ...
  if echo "$SECRETS_JSON" | node -e "JSON.parse(require('fs').readFileSync('/dev/stdin','utf8'))" 2>/dev/null; then
